...
class PieChartWidget extends StatelessWidget {
  const PieChartWidget({super.key, required this.item});
  ...
  Widget build(BuildContext context) {
    ...
    final List<dynamic> categories = item['data']?['categories'] ?? [];
        ...
        children: [
          ...
          Row(
            ...
            children: [
              ...
              Get.find<HomeController>().isfullScreenPieChart.value
                  ? IconButton(
          ...
          Text(item['description'] ?? '', style: TextStyle(color: Colors.black.withOpacity(0.8), fontSize: 14)),
            ...
            child: SfCircularChart(
              ...
              series: <CircularSeries>[
                PieSeries<Map<String, dynamic>, String>(
                  dataSource: List<Map<String, dynamic>>.from(categories),

                  xValueMapper: (data, _) => data['name']?.toString() ?? '',

                  yValueMapper: (data, _) => (data['value'] as num?)?.toDouble() ?? 0,

                  dataLabelMapper: (data, _) => '${data['name']} (${data['value']}%)',
